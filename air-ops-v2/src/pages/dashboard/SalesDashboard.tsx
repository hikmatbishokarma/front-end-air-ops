...
const SalesDashboard = () => {
  ...
  const [filter, setFilter] = useState({isLatest: {is: true}, or: [{status: {eq: "QUOTE"}}]});
 ...
 // const categories = ["Quote", "Invoice", "Cancelled", "Revenue"];
  ...
  const categories = [{status:["Quote"],name:"Quotes"}, {status:["Tax Invoice","Proforma Invoice"],name:"Invoices"}, {status:["Cancelled"],name:"Cancellations"}, {status:[],name:'Revenue'}];
    ...
    <div style={{ padding: 20 }}>
      ...
      <Grid container justifyContent="space-between" alignItems="center" style={{ marginTop: 20 }}>
       ...
       {(selectedTab == "Quotes" || selectedTab == "Invoices") &&(
         <Button variant="contained" color="primary" onClick={() => handelCreate(selectedTab)}>Create {selectedTab.slice(0, -1)}</Button>
       )}
      </Grid>
          ...
          <Grid item xs={3} key={item.name}>
            ...
            >
              <CardContent>
                ...
                <Typography variant="body1">{salesDashboardData?.summary[item.name.toLowerCase()]}</Typography>
              </CardContent>
            </Card>
          </Grid>
        ))}
